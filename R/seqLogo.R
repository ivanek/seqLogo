...
seqLogo <- function(pwm, ic.scale=TRUE,
    ...
    # correct default color setting if alphabet set to RNA
    if (alphabet=="RNA") {
        if ("T" %in% names(fill)) {
    ...
    if (!is.character(fill) || length(fill)!=nrow(pwm) ||
        !all(rownames(pwm) %in% names(fill)) || !all(.isValidColor(fill))) {
        stop("colors must be named character vector of length ",
             sprintf("%d, with names %s (matching the rownames of pwm)",
             nrow(pwm), paste(rownames(pwm), collapse=", ")),
    ...
    for (j in seq_len(npos)){
        ...
        for (i in seq_len(4)){
            ...
            if (ht>0) letters <- addLetter(letters,letter,x.pos,y.pos,ht,wt,fill)
            y.pos <- y.pos + ht + 0.01
        }
        x.pos <- x.pos + wt
    }
    ...
    if (xaxis){
        grid.xaxis(at=seq(0.5,ncol(pwm)-0.5),label=seq_len(ncol(pwm)), gp=gpar(fontsize=xfontsize))
