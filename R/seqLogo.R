...
seqLogo <- function(pwm, ic.scale = TRUE, xaxis = TRUE, yaxis = TRUE, xfontsize = 15, 
    ...
    if (!is.character(fill) || length(fill) != nrow(pwm(pwm)) || !all(rownames(pwm(pwm)) %in% 
        ...
        stop("\"fill\" must be named character vector of length ", 
             sprintf("%d, with names %s (matching the rownames of pwm)", 
            nrow(pwm(pwm)), paste(rownames(pwm(pwm)), collapse = ", ")), "and valid colors as values")
    }
    ...
    grid.polygon(x = unit(letters$x, "native"), y = unit(letters$y, "native"), id = letters$id, 
        gp = gpar(fill = letters$fill, col = "transparent"))
    if (xaxis) {
        grid.xaxis(at = seq(0.5, ncol(pwm(pwm)) - 0.5), label = seq_len(ncol(pwm(pwm))), 
